...
export class AppComponent {
  ...
  onLogout() {
    // Importation des bases de données internes
    this.storage.get('BT').then((val) => {
      this.tab = val;
      console.log('BT123 ', this.tab);

      this.storage.get('BTAwait').then((valeur) => {
      this.tabAwait = valeur;

      // Verrification si la BD Await est vide ainsi que la Bd en cours
      console.log(this.tabAwait.length === 0 );
      console.log(this.tab.length === 0 );

      if (this.tabAwait.length === 0 && this.tab.length === 0) {
        this.authService.logout();
        this.router.navigateByUrl('/auth');
        // Permet de vider la base de donnée internet à l'app
      ...
      // S'il y a des données  en attente à envoyer
      if (this.tabAwait.length !== 0) {
        ...
        this.alertCtrl.create(
          ...
          .then(alertEl => {
            ...
            // Verrification d'un éventuelle bon de travail en cours
   ...
   // Fonction qui envoie les donnée en attente à la base de donnée
  sendBT() {
